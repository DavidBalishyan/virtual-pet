(() => {
  ...
  // song buttons: each carries a data-song id the device maps to a built-in tune
  document.querySelectorAll('[data-song]').forEach((btn) => {
    btn.addEventListener('click', () => {
      sendCommand({ action: 'playSong', song: parseInt(btn.getAttribute('data-song'), 10) });
    });
  });
  const stopSongBtn = document.querySelector('[data-stop-song]');
  if (stopSongBtn) {
    stopSongBtn.addEventListener('click', () => sendCommand({ action: 'stopSong' }));
  }
